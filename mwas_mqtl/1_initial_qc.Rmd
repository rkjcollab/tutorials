...
#Load input data
intensities <- read.delim("../project-files/compound_intensities_init.txt", stringsAsFactors = F)
sample.info <- read.delim("../project-files/sample_info.txt", stringsAsFactors = F)
compound.info <- read.delim("../project-files/compound_info.txt", stringsAsFactors = F) #Not used anywhere but let's understand the information in here
```
...
The dashed horizontal and vertical lines denote the specificied number of SD away from the mean, to be used for identifying outliers

##### To change

* Based on the scree plot above, modify the below if necessary to include lower order principal component plots (by setting the x and y parameters of the biplot object)
* Change the nr.sd variable to the number of SD from the mean that should be used as threshold to identify outlier samples
...
Change the below according to the thresholds decided from the PCA scatterplots above
...
ggplot(avg.intensity.frame , aes(x=injection_order, y=mean_intensity, shape=batch, color=class)) +
  geom_point() +
  scale_shape_manual(values=1:8)
```
...
#For each batch, subtract the start_injection_order-1, so that the injection order all starts with 1 in each day
for (batch in unique(long.frame$batch)) {
  min.order <- min(long.frame$injection_order[long.frame$batch == batch])-1
  long.frame$day_order[long.frame$batch == batch] <- long.frame$injection_order[long.frame$batch == batch] - min.order
}
...
* The line that sets compound.cols may need to change according to the naming convention in the compound intensities file; the current code assumes that they all start with C.
...
write.table(compound.failure.frame,
             "../output-files/compound_QC_failures.txt",
             sep="\t", quote=F, row.names=F, col.names=T)
...
write.table(intensities,
             "../output-files/compound_intensities_clean.txt",
             sep="\t", quote=F, row.names=F, col.names=T)
